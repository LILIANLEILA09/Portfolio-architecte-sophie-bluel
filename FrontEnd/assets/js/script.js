...
function setupLoginFilterEvents(allWorks) {
  document.getElementById("login-filters").addEventListener("click", e => {
    ...
    const filteredWorks = filterValue === "all" ? allWorks : allWorks.filter(work => work.categoryId == filterValue);
...
// 1. Configuration
const CONFIG = {
  API_URL: window.location.hostname === "localhost" ? "http://localhost:5678" : "https://votre-api-en-production.com",
